window.AdminPozos = {
  ...
  priorityData(row){
    ...
    if(!row.latest){
      score += 70;
      reasons.push('Sin reportes registrados');
    }else if(ageHours !== null && ageHours >= 72){
      score += 50;
      reasons.push(`Sin visita hace ${Math.floor(ageHours / 24)} días`);
    }else if(ageHours !== null && ageHours >= 24){
      score += 35;
      reasons.push(`Sin visita hace ${Math.floor(ageHours)} horas`);
    }else if(ageHours !== null && ageHours >= 8){
      score += 15;
      reasons.push('Más de 8 horas sin visita');
    }

    if(row.alarms.length){
      score += Math.min(40, row.alarms.length * 15);
      reasons.push(
        `${row.alarms.length} ${
          row.alarms.length === 1 ? 'alarma' : 'alarmas'
        }`
      );
    }
    ...
    if(row.status.key === 'closed'){
      score += 20;
      reasons.push('Último estatus cerrado');
    }

    if(row.status.key === 'intermittent'){
      score += 15;
      reasons.push('Pozo intermitente');
    }

    if(!row.gps && row.latest){
      score += 10;
      reasons.push('Último reporte sin GPS');
    }

    if(!row.estatus && row.latest){
      score += 8;
      reasons.push('Sin estatus operativo');
    }
