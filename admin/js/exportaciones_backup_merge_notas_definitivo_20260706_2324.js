    ...
    for(const dia of dias.slice(0,1)){
      ...
      coRows.slice(0,21).forEach((r, i) => {
        ...
        if(tipo === 'NOTA' || tipo === 'CABEZAL' || tipo === 'ESTACION'){
          // Quitar combinación para que el texto salga de izquierda a derecha como Excel normal
          ...
          // La nota inicia en ESTATUS y corre hacia la derecha
          ws.getCell(`E${row}`).value = this.cortarTxt(this.textoNota(r), 90);
          ...
          // Bloqueador invisible: evita que la nota invada Observaciones
          ws.getCell(`O${row}`).value = ' ';
          ws.getCell(`O${row}`).alignment = {
            horizontal:'center',
            vertical:'middle',
            wrapText:false,
            shrinkToFit:false
        ...
        ws.getCell(`O${row}`).value = this.cortarTxt(this.obsReal(r), 55);
        ws.getCell(`O${row}`).alignment = { horizontal:'left', vertical:'middle', wrapText:false, shrinkToFit:false };
        });
