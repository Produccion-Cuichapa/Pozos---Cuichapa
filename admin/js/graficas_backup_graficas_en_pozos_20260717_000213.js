window.AdminGraficas = {
  ...
  allWellRows(){
     ...
     * usamos exactamente la misma construcción de datos
     * que ya funciona en la sección Pozos.
     */
    if(
      !window.AdminPozos ||
    ...
    const rows = AdminPozos.buildRows();
    ...
    if(!rows.length){
      const firebaseReports =
        Array.isArray(
          ...
          ? window.AdminFirebase.reportes.length
          : 0;
      ...
      container.innerHTML = `
        <div class="charts-well-empty">
          ...
          <span>
            ${
              firebaseReports === 0
                ? 'Esperando los reportes de Firebase...'
                : this.searchText
          ...
          <small>
            Reportes en Firebase: ${firebaseReports}<br>
            Pozos con historial: ${allRows.length}
          </small>
        </div>
      `;
  ...
  openSelectedWell(){
    ...
    /*
     * Se reutiliza la misma apertura del módulo Pozos.
     * No se reconstruye manualmente el expediente.
     */
    if(
      window.AdminPozos &&
      typeof AdminPozos.openWellExpedient === 'function'
    ){
      AdminPozos.openWellExpedient(
        row.sourceRow || row
      );
