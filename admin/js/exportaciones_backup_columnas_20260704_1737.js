window.AdminExportaciones = {
  ...
  async generarDiario(){
    ...
    coRows.slice(0,21).forEach((r, i) => {
      ...
      ws.getCell(`B${row}`).value = AdminUtils.placeText(r);
      ws.getCell(`C${row}`).value = this.timeFromRow(r);
      ws.getCell(`D${row}`).value = r.co?.estatus || p.estatus || '';
      ws.getCell(`E${row}`).value = r.co?.fluye || p.fluye || '';
      ws.getCell(`F${row}`).value = r.co?.sap || p.sap || '';
      ws.getCell(`G${row}`).value = r.co?.estrangulador || p.estrangulador || '';
      ws.getCell(`H${row}`).value = r.co?.ptp || p.ptp || '';
      ws.getCell(`I${row}`).value = r.co?.ldd || p.ldd || '';
      ws.getCell(`J${row}`).value = r.co?.ptr || p.ptr || '';
      ws.getCell(`K${row}`).value = r.co?.epm || p.epm || '';
      ws.getCell(`L${row}`).value = r.co?.carrera || p.carrera || '';
      ws.getCell(`O${row}`).value = AdminUtils.obsText(r);
    ...
    nivelRows.slice(0,21).forEach((r, i) => {
      ...
      ws.getCell(`B${row}`).value = AdminUtils.placeText(r);
      ws.getCell(`C${row}`).value = this.timeFromRow(r);
      ws.getCell(`D${row}`).value = r.nivel?.ctm || r.ctm || '';
