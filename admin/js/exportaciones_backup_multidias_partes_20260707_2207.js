window.AdminExportaciones = {
  ...
  async generarDiario(){
    ...
    // Crear TODAS las hojas primero desde la plantilla limpia,
    // antes de llenar datos, para evitar arrastre entre días.
    const hojas = dias.map((dia, idx) => {
      const name = this.safeSheetName(dia);
      if(idx === 0){
        templateWs.name = name;
        return { dia, ws: templateWs };
      }
      return { dia, ws: this.clonarHojaFormato(templateWs, wb, name) };
    });
