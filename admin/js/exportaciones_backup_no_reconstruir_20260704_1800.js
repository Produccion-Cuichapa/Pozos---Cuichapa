window.AdminExportaciones = {
  ...
  obsReal(r){
    let raw = String(r.observaciones || r.observacion || r.obs || '').trim();

    if(raw && !raw.includes('*REPORTE') && !raw.includes('GPS:') && !raw.includes('Estatus:')){
      return raw;
    }
    ...
    if(/^(sin observ|ninguna|n\/a|na)$/i.test(out)) return '';
    return out;
  },
  ...
  async generarDiario(){
    ...
    const base = await this.loadTemplate('../templates/Book.xlsx');
    const baseSheet = base.worksheets[0];

    const wb = new ExcelJS.Workbook();

    for(const dia of dias){
      const temp = await this.loadTemplate('../templates/Book.xlsx');
      const ws = temp.worksheets[0];
      ...
      coRows.slice(0,21).forEach((r, i) => {
        ...
        ws.getCell(`N${row}`).value = this.obsReal(r);
        ws.getCell(`N${row}`).alignment = { wrapText:true, vertical:'middle', horizontal:'center' };
      });
      ...
      const newWs = wb.addWorksheet(ws.name, {
      ...
      ws.mergedCells?.forEach?.(m => newWs.mergeCells(m));
    }

    const file = `Diario_${rec.replace(/\s+/g,'_')}_${desde}_a_${hasta}.xlsx`;
    await this.downloadWorkbook(wb, file);
    box.innerHTML = `<b>${allRows.length}</b> reportes exportados en ${dias.length} hoja(s), una por día.`;
  },
