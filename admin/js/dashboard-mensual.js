...
/NIVEL\s+(?:DE\s+)?(?:FRAC\s*TANK|PRESA\s*MET[ÁA]LICA)/i.test(message);
  ...
  function calculate(rows, range){
    ...
    rows.forEach(report => {
      ...
      /*
       * La nueva pantalla de Aforo genera un reporte
       * independiente, sin encabezado REPORTE DE VISITA.
       */
      const isIndependentAforo =
        Boolean(
          window.AforoUtils &&
          AforoUtils.esReporteAforoIndependiente(report)
        );
      ...
      /*
       * Aforo generado desde la nueva pantalla:
       * suma únicamente en AFORO y respeta el rango activo.
       * No incrementa SUPER.
       */
      if(isIndependentAforo){
        totals.aforo += 1;
      ...
      /*
       * Reporte de visita con la actividad Aforo seleccionada.
       * El independiente ya fue contabilizado arriba.
       */
      if(
        !isIndependentAforo &&
        window.AforoUtils &&
        AforoUtils.esActividadAforo(report)
      ){
        totals.aforo += 1;
      }
  ...
  window.AdminDashboardMensual = {
    ...
    listen(){
      ...
      if(this.reportesQuery){
        this.reportesQuery.off();
      }
      ...
      this.rows = [];
      this._rowsById = Object.create(null);

      const range = this.getActiveRange();

      this.reportesQuery = this.db
        .ref('reportes')
        .orderByChild('fecha')
      ...
      const refresh = () => {
        ...
        this._renderTimer = setTimeout(() => {
          this.rows = Object.values(this._rowsById);
          this.render();
        }, 100);
      };

      this.reportesQuery.on('child_added', child => {
        this._rowsById[child.key] = {
          id: child.key,
          ...(child.val() || {})
        };
        refresh();
      ...
      this.reportesQuery.on('child_changed', child => {
        this._rowsById[child.key] = {
          id: child.key,
          ...(child.val() || {})
        };
      ...
      this.reportesQuery.on('child_removed', child => {
        delete this._rowsById[child.key];
        refresh();
      });
    },
  ...
  const MONTHLY_ACTIVE_WORKERS = [
    ...
    },
    {
      ...
      aliases: [
        /*
         * Los reportes actuales guardan al recorredor
         * simplemente como "Luis".
         */
