window.AdminExportaciones = {
  ...
  textoNota(r){
    ...
    return this.cortarTxt(txt.replace(/[🌎🗺️]/g,'').trim(), 110);
  },
    ...
    for(const dia of dias.slice(0,1)){
      ...
      coRows.slice(0,21).forEach((r, i) => {
        ...
        if(tipo === 'NOTA' || tipo === 'CABEZAL' || tipo === 'ESTACION'){
          ...
          try{ ws.unMergeCells(`E${row}:N${row}`); }catch(e){}
          ws.mergeCells(`E${row}:N${row}`);

          const c = ws.getCell(`E${row}`);
          c.value = this.textoNota(r);
          c.alignment = {
            horizontal:'center',
            vertical:'middle',
            wrapText:false,
          ...
          // No tocar columna O; conserva formato original de Observaciones
          return;
        }
        ...
        ws.getCell(`I${row}`).value = ((r.co?.ptp || p.ptp || '').toString().replace(/kg\/cm²/gi,'').replace(/kg\/cm2/gi,'').replace(/kg\/cm\^2/gi,'')).toString().replace(/kg\/cm²/gi,'').replace(/kg\/cm2/gi,'').replace(/kg\/cm\^2/gi,'');
        ws.getCell(`J${row}`).value = ((r.co?.ldd || p.ldd || '').toString().replace(/kg\/cm²/gi,'').replace(/kg\/cm2/gi,'').replace(/kg\/cm\^2/gi,'')).toString().replace(/kg\/cm²/gi,'').replace(/kg\/cm2/gi,'').replace(/kg\/cm\^2/gi,'');
        ws.getCell(`K${row}`).value = ((r.co?.ptr || p.ptr || '').toString().replace(/kg\/cm²/gi,'').replace(/kg\/cm2/gi,'').replace(/kg\/cm\^2/gi,'')).toString().replace(/kg\/cm²/gi,'').replace(/kg\/cm2/gi,'').replace(/kg\/cm\^2/gi,'');
