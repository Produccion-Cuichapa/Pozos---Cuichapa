window.AdminReportes = {
  ...
  normalizedMode(row){
    /*
     * La nueva función de Aforo manda sobre el modo heredado.
     * Corrige registros guardados accidentalmente como C.O.
     */
    if(
      window.AforoUtils &&
      AforoUtils.esReporteAforoIndependiente(row)
    ){
      return {
        key: 'aforo',
        label: 'Aforo'
  ...
  operationalStatus(row){
    ...
    /*
     * ESTATUS OPERATIVO — PRIORIDAD:
     *
     * 1. Maniobra realizada: APERTURA / CIERRE
     * 2. Estado actual: Abierto / Cerrado / Intermitente
     * 3. Estado/Estatus del pozo dentro del mensaje
     * 4. Campos estructurados del reporte
     *
     * Una maniobra tiene prioridad porque representa
     * el evento operativo realizado en esa visita.
     */
    ...
    const normalizeStatus = value => {
      ...
      if(
        raw.includes('intermitente')
      ){
        return 'Intermitente';
      }

      if(
        raw.includes('abierto') ||
     ...
     *
     * Ejemplos:
     * Maniobra realizada: APERTURA
     * Maniobra realizada: CIERRE
     */
    const maniobra =
      message.match(
     ...
     * ESTADO ACTUAL
     *
     * Este es el formato que actualmente
     * manda la aplicación.
     */
    const estadoActual =
      message.match(
  ...
  render(){
    ...
    const renderRow = row => {
      ...
      let estatus = "";
        ...
        try{
          estatus = this.operationalStatus(row);
        }catch(error){
          console.error("[Reportes] Error leyendo estatus:", row?.id, error);
        }
    ...
    const renderNextChunk = () => {
      ...
      const html = rows
        .slice(rendered, end)
        .map(function(row){ try{ return renderRow(row); }catch(error){ console.error("[Reportes] FILA FALLÓ:", row?.id, error); return ""; } })
        .join('');
