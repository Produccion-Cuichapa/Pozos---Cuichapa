  ...
  openDetail(type, item){
    ...
    document.getElementById('detailBody').innerHTML = `
      ...
      <div class="detail-grid">
        ...
        <div class="detail-box"><span>GPS</span><b>${lat && lon ? 'Disponible' : 'Sin GPS'}</b></div>
        <div class="detail-box"><span>Fotos</span><b>${fotos.length || item.nFotos || 0}</b></div>
      </div>

      <div class="detail-section">
        <h3>Control operativo</h3>
        <div class="detail-grid">
          <div class="detail-box"><span>Estatus</span><b>${u.escapeHtml(co.estatus || item.estatus || '-')}</b></div>
          <div class="detail-box"><span>Fluye</span><b>${u.escapeHtml(co.fluye || item.fluye || '-')}</b></div>
          <div class="detail-box"><span>SAP</span><b>${u.escapeHtml(co.sap || item.sap || '-')}</b></div>
          <div class="detail-box"><span>Estrangulador</span><b>${u.escapeHtml(co.estrangulador || item.estrangulador || '-')}</b></div>
          <div class="detail-box"><span>PTP</span><b>${u.escapeHtml(co.ptp || item.ptp || '-')}</b></div>
          <div class="detail-box"><span>LDD</span><b>${u.escapeHtml(co.ldd || item.ldd || '-')}</b></div>
          <div class="detail-box"><span>PTR</span><b>${u.escapeHtml(co.ptr || item.ptr || '-')}</b></div>
          <div class="detail-box"><span>EPM</span><b>${u.escapeHtml(co.epm || item.epm || '-')}</b></div>
          <div class="detail-box"><span>Carrera</span><b>${u.escapeHtml(co.carrera || item.carrera || '-')}</b></div>
        </div>
      </div>
      ...
      <div class="detail-section">
        ...
        <div class="gps-card">
          <div>
            <b>${lat && lon ? `${u.escapeHtml(lat)}, ${u.escapeHtml(lon)}` : 'Sin coordenadas disponibles'}</b>
