window.AdminExportaciones = {
  ...
  textoNota(r){
    ...
    return this.cortarTxt(this.limpiarGpsTexto(txt.replace(/[🌎🗺️]/g,'').trim()), 110);
  },
  ...
  async generarDiario(){
    ...
    for(const dia of dias.slice(0,1)){
      ...
      coRows.slice(0,21).forEach((r, i) => {
        ...
        ws.getCell(`O${row}`).alignment = { horizontal:'center', vertical:'middle', wrapText:false, shrinkToFit:false };
        ws.getCell(`P${row}`).value = '';
        ws.getCell(`P${row}`).alignment = { horizontal:'center', vertical:'middle', wrapText:false, shrinkToFit:false };
        });
