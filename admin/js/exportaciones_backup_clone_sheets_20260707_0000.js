window.AdminExportaciones = {
  ...
  async generarDiario(){
    ...
    const templateModel = JSON.parse(JSON.stringify(wb.worksheets[0].model));
    ...
    for(const [idx, dia] of dias.entries()){
      ...
      if(idx === 0){
        ...
        ws = wb.addWorksheet(this.safeSheetName(dia));
        ws.model = JSON.parse(JSON.stringify(templateModel));
